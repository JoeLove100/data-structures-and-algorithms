...
class Graph:
    ...
    def _add_strongly_connected_components(self,
        ...
        for neighbour_key in current_node.neighbours:
            ...
            self._update_current_lowest_reachable_index(neighbour_node, current_node, all_scc)
    ...
    def get_topological_ordering(self):
        """
        Tarjan's algorithm gives us a reverse topological
        sort in linear time as a byproduct of finding
        the SCCs
        """

        all_scc = self._get_strongly_connected_components()
        flattened_scc = [key for scc in all_scc for key in scc]
        return flattened_scc[::-1]
...
if __name__ == "__main__":
    ...
    topological_order = graph.get_topological_ordering()
